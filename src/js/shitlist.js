...
var renderShitlist = function() {
    var onRead = function(response) {
        cachedShitlist = response.shitlist.trim();
        uDom.nodeFromId('whitelist').value = cachedShitlist + '\n';

        shitlistChanged();
    };
    messaging.send('dashboard', { what: 'getShitlist' }, onRead);
};
...
var exportShitlistToFile = function() {
    var val = uDom('#whitelist').val().trim();
    if ( val === '' ) {
        return;
    }
    var now = new Date();
    var filename = vAPI.i18n('shitlistExportFilename')
...
var applyChanges = function() {
    cachedShitlist = uDom.nodeFromId('whitelist').value.trim();
    var request = {
        what: 'setShitlist',
        shitlist: cachedShitlist
    };
    messaging.send('dashboard', request, renderShitlist);
};
...
uDom('#exportWhitelistToFile').on('click', exportShitlistToFile);
uDom('#whitelist').on('input', shitlistChanged);
